     ...
     <form action="{{route('company.update',['id'=>$company->id])}}" method="POST" enctype="multipart/form-data">
      ...
      @endif
        ...
        <div class="pb-3">
          <div class="py-3">
            <p>Company logo</p>
            <img id="logo_image" src="{{ asset($company->logo) }}" width="80px" alt="">
          </div>
        ...
        <div class="pb-3">
          <div class="py-3">
            ...
            <img id="cover_image" src="{{ asset($company->cover_img) }}" width="200px" class="img-fluid" alt="">
          </div>
          <div class="custom-file">
        ...
        <div class="form-group">
          ...
          <input type="url" placeholder="https://www.examplecompany.com" class="form-control @error('website') is-invalid @enderror" name="website" value="{{ old('website') ?? $company->website }}" required>
          @error('website')
              <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
              </span>
